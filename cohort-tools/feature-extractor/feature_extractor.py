...
class FeatureExtractor():
    ...
    def __baseline_covariate_data__(self, plp_data):
        ...
        baseline_covariate_data = df_long.pivot_table(index= 'subjectId',  columns = 'covariateName',  values =  'covariateValue').fillna(0)
    ...
    def __r_temporal_covariate_data__(self, subject_ids):
        ...
        sql = """ 
              ...
              FROM (SELECT *
                    ...
                    AND c.subject_id IN ({})) c,
                    
                    {}.visit_detail v,
                    (SELECT *,
                     FROM {}.measurement m,
                     WHERE m.person_id IN ({}),
                     AND m.measurement_concept_id IN ({})) m
              ...
              """.format(self._cohort_details['target_cohort_database_schema'],
                         self._cohort_details['target_cohort_table'],
                         self._cohort_details['target_cohort_id'],
                         subject_ids,
                         self._cohort_details['cdm_database_schema'],
                         self._cohort_details['cdm_database_schema'],
                         subject_ids,
                         self._temporal_covariate_setting_options['included_covariate_ids'])

        
        path_to_covariate_data = self._model_training_settings['path'] + 'covariates.csv'
        
        command = """\copy {} to {} with csv header""".format(sql, path_to_covariate_data)
        
        #execute
        command = """PGPASSWORD={} psql -h {} -p {} -d {} -U {} -c '\\copy {} to {} with csv header'""".format(
                     self._db_connection_details['password'], 
                     self._db_connection_details['hostname'], 
                     self._db_connection_details['port'], 
                     self._db_connection_details['dbname'], 
                     self._db_connection_details['user'], 
                     sql,
                     path_to_covariate_data)
        ...
        r_temporal_covariate_data = get_temporal_covariate_data(hostname = self._db_connection_details['hostname'], 
                                                 ...
                                                 path = self._model_training_settings['path'])
    ...
    def __train_val_split__(self, temporal_covariate_data, outcome_per_stay):
    ...
    def __output_dir__(self):
        ...
        if not os.path.exists(self._model_training_settings['path']):
            os.makedirs(self._model_training_settings['path'])
            
        out_dir = Path(self._model_training_settings['path'])
        return out_dir
    ...
    def __training_setup__(self):
        ...
        X_train, X_val, y_train, y_val = self.__train_val_split__(temporal_covariate_data_wide, 
                                                                  outcomes_per_stay)
