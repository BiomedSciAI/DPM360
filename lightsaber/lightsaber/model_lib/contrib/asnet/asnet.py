...
"""
Main module file for Atkinson(A)-Shiffrin(S) inspired Neural Network
...
from asnet.memory import ExplicitMem, BlurredMem, MixGate
...
class ASNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.short_term = ExplicitMem(self.cell_size, self.n_slots, self.n_reads, 
                                      self.hidden_size, self.tau)
        self.long_term = MemN2N(self.cell_size, self.n_slots, self.n_reads)
    
        #  self.out_gate = MixGate(self.cell_size)
        self.out = nn.Linear(self.cell_size, self.cell_size)
    ...
    def forward(self, input, hx=(None, None, None, None, None), save_trace=False):
        ...
        ctl_hidden, exp_hidden, blr_hidden, em_index, l_read = self.init_hidden(batch_size,
                                                                                hx)
